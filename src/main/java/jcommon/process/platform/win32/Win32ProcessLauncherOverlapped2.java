...
////        );
//
//      final IOCPBUFFER.ByReference iocpBuffer = new IOCPBUFFER.ByReference();
//      final OVERLAPPED_WITH_BUFFER_AND_STATE o = overlapped_pool.requestInstance(state);
//      o.iocpBuffer = iocpBuffer;
//      iocpBuffer.buffer = memory_pool.requestSlice();
//      iocpBuffer.bufferSize = 1024;
//      iocpBuffer.sequenceNumber = process_info.readSequenceNumber;
//
//
...
//
//            //synchronized (memory_pool.getLock()) {
//              ByteBuffer bb = iocpBuffer.buffer.getByteBuffer(0, dwNumberOfBytesTransferred);
//
//              process_info.notifyStdOut(bb, dwNumberOfBytesTransferred);
//
//
//              PinnableCallback.unpin(this);
//              PinnableMemory.unpin(iocpBuffer.buffer);
...
//        });
//
//        if (!ReadFileEx(pipe, iocpBuffer.buffer, iocpBuffer.bufferSize, o, callback) /*|| lp_bytes_read.getValue() == 0*/) {
//          int err = GetLastError();
//          switch(err) {
//            case ERROR_IO_PENDING:
//              //Normal operation. Nothing to see here.
