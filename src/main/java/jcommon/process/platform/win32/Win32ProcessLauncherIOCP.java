...
//    @Override
//    public void process(int state, IOCPBUFFER iocpBuffer, ProcessInformation processInformation, Pointer completionKey, Pointer pOverlapped, IntByReference pBytesTransferred, OverlappedPool overlappedPool, MemoryPool memoryPool, IOCompletionPort<ProcessInformation> iocp) throws Throwable {
//      }
//
//    @Override
//    public void process(int state, int bytesTransferred, OVERLAPPED_WITH_BUFFER_AND_STATE ovl, ProcessInformation processInformation, Pointer completionKey, Pointer pOverlapped, IntByReference pBytesTransferred, Object overlappedPool, Object memoryPool, IOCompletionPort<ProcessInformation> iocp) throws Throwable {
//      completion_thread(state, bytesTransferred, ovl, processInformation, completionKey, pOverlapped, pBytesTransferred, overlappedPool, memoryPool, iocp);
//    }
//  });
//
...
//    , OP_CLOSED              = 20
//    , OP_EXITTHREAD          = 21 //OVERLAPPED_WITH_BUFFER_AND_STATE.STATE_EXITTHREAD
//  ;
//private static final AtomicInteger READS = new AtomicInteger(0);
//  private static void completion_thread(int state, int bytesTransferred, OVERLAPPED_WITH_BUFFER_AND_STATE ovl, ProcessInformation process_info, Pointer completion_key, Pointer ptr_overlapped, IntByReference ptr_bytes_transferred, Object overlapped_pool, Object memory_pool, IOCompletionPort<ProcessInformation> iocp) throws Throwable {
...
//          read(iocp, process_info, completion_key, null /*overlapped_pool*/, null /*memory_pool*/, process_info.stdout_child_process_read, OP_STDOUT_READ);
//          read(iocp, process_info, completion_key, null /*overlapped_pool*/, null /*memory_pool*/, process_info.stderr_child_process_read, OP_STDERR_READ);
//
...
//        IOCPBUFFER iocpBuffer = ovl.iocpBuffer;
//        bytes_transferred = Math.max(0, Math.min(ptr_bytes_transferred.getValue(), iocpBuffer.bufferSize));
//        iocpBuffer.use(bytes_transferred);
//
...
//        while((iocp_buffer = process_info.nextReadBuffer(iocpBuffer)) != null) {
//          process_info.notifyStdOut(iocpBuffer.buffer.getByteBuffer(0, iocp_buffer.used), iocp_buffer.used);
//          process_info.incrementReadSequenceNumber();
//        }
//
...
//    DuplicateHandle(GetCurrentProcess(), tmp = ptr_stdout_child_process_read.getValue(), GetCurrentProcess(), ptr_stdout_child_process_read, 0, false, DUPLICATE_SAME_ACCESS);
//    CloseHandle(tmp);
//
...
//    DuplicateHandle(GetCurrentProcess(), tmp = ptr_stderr_child_process_read.getValue(), GetCurrentProcess(), ptr_stderr_child_process_read, 0, false, DUPLICATE_SAME_ACCESS);
//    CloseHandle(tmp);
//
...
//    DuplicateHandle(GetCurrentProcess(), tmp = ptr_stdin_child_process_write.getValue(), GetCurrentProcess(), ptr_stdin_child_process_write, 0, false, DUPLICATE_SAME_ACCESS);
//    CloseHandle(tmp);
//
...
//    }
//
////    PinnableStruct.pin(stdout_child_process_read.getPointer());
////    PinnableStruct.pin(stderr_child_process_read.getPointer());
////    PinnableStruct.pin(stdin_child_process_read.getPointer());
////    PinnableStruct.pin(process_info.process.getPointer());
//
...
//      o.state = op;
//      o.buffer = PinnableMemory.pin(1024);
//      o.bufferSize = 1024;
//      o.sequenceNumber = process_info.readSequenceNumber;
//      o.write();
//
...
//          case ERROR_IO_PENDING:
//            //Normal operation. Nothing to see here.
//            process_info.readSequenceNumber = (process_info.readSequenceNumber + 1) % ProcessInformation.MAX_SEQUENCE_NUMBER;
//            return true;
//        }
//
//        //For abnormal completion, clean up pinned instances and re-evaluate
