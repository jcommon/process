...
build() {
  ...
  strip=$OPT_DIR/$cross_gcc/bin/${cross_gcc}-strip
  ...
  "$gcc" -Os -ffunction-sections -fdata-sections -I"$SRC_DIR" -o "$obj" -c "$src"
  "$gcc" -s -Wl,--gc-sections -static -static-libgcc -o "$out" "$obj"
  "$strip" --strip-all --remove-section=.comment --remove-section=.note "$out"
